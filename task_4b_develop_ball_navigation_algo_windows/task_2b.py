...
def get_vision_sensor_image():
    ...
    ##############	ADD YOUR CODE HERE	##############
    res, sensorHandle = sim.simxGetObjectHandle(
    ...
    return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(
        client_id, sensorHandle, 0, sim.simx_opmode_blocking)
    ##################################################
...
def send_data(cid, maze_array):
    ...
    return_code, retInts, retFloats, retStrings, retBuffer = sim.simxCallScriptFunction(cid, 'top_plate_respondable_1', sim.sim_scripttype_customizationscript,
                                                                                'receiveData', maze_array, [], ['Sending mazearray to coppeliasim'], inputBuffer, sim.simx_opmode_blocking)
...
if __name__ == "__main__":
    ...
    if ((client_id != -1) and (maze_array_generated_flag == 1)):
        ...
        try:
            ...
            return_code = send_data(client_id, maze_array)
                    ...
                    if ((return_code == sim.simx_return_ok) and (len(image_resolution) == 2) and (len(vision_sensor_image) > 0)):
                        ...
                        try:
                            ...
                            if (type(transformed_image) is np.ndarray):
                                ...
                                generated_img_file_path = generated_dir_path + \
                                    'result_maze0' + str(file_num) + '.jpg'
                                ...
                                return_code = cv2.imwrite(
                                    generated_img_file_path, transformed_image)

                                if return_code == True:
                                    print('\nTransformed maze image from CoppeliaSim: ' + str(
                                        generated_img_file_path) + ' was saved in \'generated_images\' folder successfully!')
