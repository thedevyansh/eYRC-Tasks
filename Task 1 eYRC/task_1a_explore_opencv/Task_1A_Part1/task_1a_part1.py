...
def scan_image(img_file_path):
    ...
    for cnt in contours:
        ...
        if len(approx) == 3:
            update_dictionary('Triangle', shape_color,
                              cv2.contourArea(cnt), cx, cy)
        elif len(approx) == 4:
            quad_type = find_quadrilateral_type(approx)
            update_dictionary(quad_type, shape_color,
                              cv2.contourArea(cnt), cx, cy)
        elif len(approx) == 5:
            update_dictionary('Pentagon', shape_color,
                              cv2.contourArea(cnt), cx, cy)
        elif len(approx) == 6:
            update_dictionary('Hexagon', shape_color,
                              cv2.contourArea(cnt), cx, cy)
        else:
            update_dictionary('Circle', shape_color,
                              cv2.contourArea(cnt), cx, cy)

    shapes = {k: v for k, v in sorted(
        shapes.items(), key=lambda item: item[1][1], reverse=True)}
