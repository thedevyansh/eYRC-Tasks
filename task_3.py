...
def init_setup(rec_client_id):
    ...
    ##############	ADD YOUR CODE HERE	##############
    global x_motor1, x_motor2, y_motor1, y_motor2
    return_code = 0
...
def control_logic(center_x, center_y):
    ...
    Kdx = 1.57 * 1.2 * timeChange / 26
    Kdy = 1.57 * 1.2 * timeChange / 26
    ...
    sim.simxSetJointTargetPosition(
        client_id, x_motor1, px, sim.simx_opmode_streaming)
    sim.simxSetJointTargetPosition(
        client_id, y_motor1, py, sim.simx_opmode_streaming)
    sim.simxSetJointTargetPosition(
        client_id, x_motor2, px, sim.simx_opmode_streaming)
    sim.simxSetJointTargetPosition(
        client_id, y_motor2, py, sim.simx_opmode_streaming)
