...
def edge_values(img):
	...
	B_left, G_left, R_left = (img[int(y / 2), 0]) #BGR of the left mid_point	
	B_top, G_top, R_top = (img[0, int(x / 2)]) #BGR of top mid_point
	B_right, G_right, R_right = (img[int(y / 2), x - 1]) #BGR of right mid_point
	B_bottom, G_bottom, R_bottom = (img[y - 1, int(x / 2)]) #BGR of bottom_midpoint
...
def applyPerspectiveTransform(input_img):
	...
	gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY) #Converting to gray

	#Masking to get the maze specifically
