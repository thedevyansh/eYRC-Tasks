...
def applyPerspectiveTransform(input_img):
    ...
    #Storing the bounding rectangles of contours
        dimensions = []
        #i = 0
        for cnts in contours:
            x, y, w, h = cv2.boundingRect(cnts)
            dimensions.append([x, y, w + x, h + y])
            #cv2.rectangle(img, (x, y), (x + w, y + h), (i, 2 * i,255), 2)
            #cv2.drawContours(img, cnts, -1, (i, 2 * i,255), 2)
            #i += 20
        ...
        for i in range(len(dimensions)):
            if(dimensions[i][0] < minx):
                minx = dimensions[i][0]
            if(dimensions[i][1] < miny):
                miny = dimensions[i][1]
            if(dimensions[i][2] > maxx):
                maxx = dimensions[i][2]
            if(dimensions[i][3] > maxy):
                maxy = dimensions[i][3]
    ...
    #Storing Contours
        for cnts in contours:
            ...
            if not (area / (1024*1024) > 0.9 and area / (1024*1024) < 1.1):
                areas.append(area)
